...
class TutoringWorkflow:
    ...
    def invoke(self, invocation: RunInvocation) -> WorkflowOutcome:
        ...
        if directive == "close":
            ...
            return WorkflowOutcome(run_status="closed", next_interaction={"status": "closed"})
        ...
        record_learning_event(self.db, ward_id=session.ward_id, event_type="tutoring.ward_attempt_recorded", source_type="tutoring_message", source_id=ward.id, payload={"tutoring_session_id": tutor.id, "study_session_id": session.id})
        ...
        self.db.add(TutoringMessage(tutoring_session_id=tutor.id, role="assistant", content=answer, hint_level=level, safety_blocked=blocked))
        return WorkflowOutcome(run_status="waiting_for_ward", next_interaction={"status": "needs_input", "content": answer, "hint_level": level, "safety_blocked": blocked})
