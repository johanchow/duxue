...
from app.infrastructure.persistence.models import BehaviorLabelConfig, BehaviorSegment, Frame, FramePrediction, GuardianWard, Report, Ward, now


# Runtime classifier configuration is deployment data, not a Python package
# resource.  The module now lives beneath the application package, so resolve it from the
# service root rather than from the former flat app/ directory.
classifier = RuleClassifier(Path(__file__).parents[3] / "classifier" / "rules.yaml")
